...
import { IBooking } from '../bookings/models/booking'
...
function prepareData(startData: DurationData[], stays: IBooking[]) {
  function incArrayValue(arr: DurationData[], field: string) {
    return arr.map((obj) =>
    ...
    <ChartBox>
      ...
      <ResponsiveContainer width={'100%'} height={240}>
        <PieChart>
          ...
          >
            {data.map((i) => (
              <Cell fill={i.color} stroke={i.color} key={i.duration} />
            ))}
          </Pie>
          <Tooltip />
          <Legend
